...
int __cdecl wmain(int argc, _In_reads_(argc) WCHAR** argv)
{
   ...
   TRYODBC(hDbc, SQL_HANDLE_DBC, SQLDriverConnect(hDbc, GetDesktopWindow(), pwszConnStr, SQL_NTS, NULL, 0, NULL, SQL_DRIVER_COMPLETE));
...
void DisplayResults(HSTMT hStmt, SQLSMALLINT cCols)
{
   ...
   do
   {
      ...
      else
      {
         ...
         for (pThisBinding = pFirstBinding; pThisBinding;
              pThisBinding = pThisBinding->sNext)
         {
            if (pThisBinding->indPtr != SQL_NULL_DATA)
            {
               wprintf(pThisBinding->fChar ? DISPLAY_FORMAT_C : DISPLAY_FORMAT, PIPE, pThisBinding->cDisplaySize, pThisBinding->cDisplaySize, pThisBinding->wszBuffer);
            }
            else
            {
               wprintf(DISPLAY_FORMAT_C, PIPE, pThisBinding->cDisplaySize, pThisBinding->cDisplaySize, L"<NULL>");
            }
         }
         wprintf(L" %c\n", PIPE);
      }
...
void AllocateBindings(HSTMT hStmt, SQLSMALLINT cCols, BINDING** ppBinding, SQLSMALLINT* pDisplay)
{
   ...
   *pDisplay = 0;
   ...
   for (iCol = 1; iCol <= cCols; iCol++)
   {
      ...
      pThisBinding->fChar = (ssType == SQL_CHAR || ssType == SQL_VARCHAR ||
      ...
      pThisBinding->cDisplaySize =
         max((SQLSMALLINT) cchDisplay, cchColumnNameLength);
...
void DisplayTitles(HSTMT hStmt, DWORD cDisplaySize, BINDING* pBinding)
{
   ...
   {
      TRYODBC(hStmt, SQL_HANDLE_STMT, SQLColAttribute(hStmt, iCol++, SQL_DESC_NAME, wszTitle,
   ...
   if (hConsole != INVALID_HANDLE_VALUE)
   {
      ...
      if (fInvert)
      {
         SetConsoleTextAttribute(hConsole,
                                 (WORD) (csbInfo.wAttributes | BACKGROUND_BLUE));
      }
      else
      {
         SetConsoleTextAttribute(hConsole,
